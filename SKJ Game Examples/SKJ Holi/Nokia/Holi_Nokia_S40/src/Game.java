 ...
 * @version 1.0.0
 *
 * Game on the Indian festival of Holi. Main character's name is Saheb.
 * We have used the FullScreen class of the Nokia UI API here.
 * Also note that everytime vibration has been called the code has been enclosed within
 * try block. This is necessary as if the user has switched off the vibration mode in his
 * handset, we will get an exception in the vibration code. Catching it is necessary for keeping the 
 * game running. Also note that instead of the soft key commands we had to create a short cut 
 * menu which comes on pressing any soft key.
 * Also although the code was written for Nokia Series 40 handset, the SKJ UI portability technique
 * helps the code to run unchanged on even a Nokia Series 60 handset.
 * Do keep in mind that this example should be compiled using a Nokia emulator and also it
 * should be executed only on Nokia handsets. On other handsets not containing the Nokia UI API
 * it would not run. We compiled it using Nokia 7210 emulator that can be downloaded from the net.
 */
...
{   
	...
	private void init()
	{
		...
		col =0;
		tnogb=0;
		s=new Sound(3500,200);
			
		// Initializing max and maxY
		maxX = this.getWidth() ;
		maxY = this.getHeight() ;
	...
	public void newGame()
	{
		// Making all sprites die except 'Saheb'
		for(int i = 1;i < 12;i++)
		{
			spriteDie(i) ;
		}
	...
	private void chanceOver()
	{
		...
		sprite[0].setVisible(false) ;	// Making Saheb invisible
	...
	private void born(int index)
	{
		...
		if(index > 8 && index < 12)		// Saheb's balloon
		{
	...
	private void collision()
	{
		for(int i = 9;i < 12;i++)
		{
			for(int j = 1;j < 9;j++ )
			{
				if(sprite[i].collidesWith(sprite[j]) == true || sprite[j].collidesWith(sprite[i]) == true)
				{
					...
					try
					{
						DeviceControl.startVibra(12,400);						
					}
		...
		// When Saheb is hit
		if(sprite[0].collidesWith(sprite[1]) == true || sprite[1].collidesWith(sprite[0]) == true)	// When Basu's balloon hits Saheb
		{
			// For vibration
			try
			{
				DeviceControl.startVibra(40,2000);				
			}
			catch (IllegalStateException e)
			{
			}
    ...
    protected  void keyPressed(int keyCode)
    {
		...
		{
			screenID = 4 ;
			sprite[0].setVisible(true) ;		// Making Saheb visible again
			gamePaused = false ;
			col=0;
			tnogb=0;
			return ;
